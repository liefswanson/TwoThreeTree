...
class TwoThreeNode:
    ...
    def __str__(self):
        return self.treeToStr(self, "")

    def treeToStr(self, node, indent=""):
        if isInterior(node):
        ...
        left   = node.treeToStr(node.left,   indent + " |")
        middle = node.treeToStr(node.middle, indent + " |")
        right  = ""
        if hasThreeChildren(node):
            right = node.treeToStr(node.right, indent + " |")

        return cargo + left + middle + right

    # TODO test this function
    # should output an ordered python list of the tree leaves
    # then that list should be printable
    def treeToList(self):
        if isInterior(self.left):
            if hasThreeChildren(self):
                output = (self.left.treeToList() +
                          self.middle.treeToList() +
                          self.right.treeToList())
            else:
                output = (self.left.treeToList() +
                          self.middle.treeToList())
            return output
